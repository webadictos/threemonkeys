 ...
 */
if( ! class_exists( 'SPREADNEWS_CMB2_Field_Spread_Post_Search_Ajax' ) ) {
	
	class SPREADNEWS_CMB2_Field_Spread_Post_Search_Ajax {
		...
		public function __construct() {
			add_action( 'cmb2_render_spread_post_search_ajax', array( $this, 'render' ), 10, 5 );
			add_action( 'cmb2_sanitize_spread_post_search_ajax', array( $this, 'sanitize' ), 10, 4 );
			add_action( 'wp_ajax_cmb_spread_post_search_ajax_get_results', array( $this, 'cmb_spread_post_search_ajax_get_results' ) );
		}
		...
		public function render( $field, $value, $object_id, $object_type, $field_type ) {	
			...
			echo $field_type->input( array( 
				...
				'name' 			=> $field_name,
				'id'			=> $field_name,
				'class'			=> 'cmb-spread-post-search-ajax',
				'value' 		=> $field_value,
				'desc'			=> false,
			...
			echo '<img src="'.admin_url( 'images/spinner.gif' ).'" class="cmb-spread-post-search-ajax-spinner" />';		

			$field_type->_desc( true, true );


			if($field->args( 'limit' ) > 1){
				echo '<ul class="cmb-spread-post-search-ajax-results" id="' . $field_name . '_results">';
		...
		public function setup_admin_scripts() {
			...
			wp_register_script( 'spreadnews-post-search-ajax', self::url( 'js/spreadnews-post-search-ajax.js' ), array( 'jquery', 'jquery-autocomplete', 'jquery-ui-sortable' ), "1.2.2.3" );
			wp_localize_script( 'spreadnews-post-search-ajax', 'psa', array(
				'ajaxurl' 	=> admin_url( 'admin-ajax.php' ),
				'nonce'		=> wp_create_nonce( 'spread_cmb_post_search_ajax_get_results' )
			) ); 
			wp_enqueue_script( 'spreadnews-post-search-ajax' );
			wp_enqueue_style( 'spreadnews-post-search-ajax', self::url( 'css/spreadnews-post-search-ajax.css' ), array(), "1.3.2.6" );
		 ...
		 */
		public function cmb_spread_post_search_ajax_get_results() {
			$nonce = $_POST['psacheck'];
			if ( ! wp_verify_nonce( $nonce, 'spread_cmb_post_search_ajax_get_results' ) ) {
				...
				if( $_POST['object'] == 'user' ){
					...
					if (!empty($results)) {
						foreach( $results as $result ){
							...
							$datas[] = apply_filters( 'spreadnews_cmb_post_search_ajax_result', array(
...
$spreadnews_cmb2_field_post_search_ajax = new SPREADNEWS_CMB2_Field_Spread_Post_Search_Ajax();
