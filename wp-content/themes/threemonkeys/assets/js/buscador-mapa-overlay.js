...
const MapaGG = (() => {
  ...
  const mapSelector = 'buscador-map';
  const currentLang = ThemeSetup.lang || 'es';
  const modalSelector = document.querySelector('#neartome-canvas');
  let modalWindow = '';
  ...
  const initModal = () => {
    modalWindow = new bootstrap.Offcanvas(modalSelector);

    modalSelector.addEventListener('hidden.bs.offcanvas', function () {
      console.log('Se ocultó');
    });

    // modal.querySelector('button.close').addEventListener('click', event => {
